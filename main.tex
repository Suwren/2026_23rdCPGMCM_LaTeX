...
问题一单电池的分层结构、主要传输过程及模型输出如\Cref{fig:p1_coupling} 所示。
\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\linewidth,height=0.78\textheight,keepaspectratio]{figures/q1_cold_start_model.pdf}
  \caption{问题一冷启动模型的电池结构、耦合过程与输出指标}
...
\Cref{fig:p1_loss} 分别给出 $-20~^{\circ}\mathrm{C}$ 和 $-25~^{\circ}\mathrm{C}$ 工况下的单电池电压、可逆电压与三项极化损失；浓差损失采用右侧纵轴。典型时刻数值见\Cref{tab:p1_loss}。可以发现：
...
\begin{figure}[H]
  \centering
  \includegraphics[width=14cm]{figures/p1_loss_uniform.pdf}
  \caption{两个低温工况的单电池电压、可逆电压与三项极化损失}
  \label{fig:p1_loss}
\end{figure}
...
问题二将问题一的一维单电池模型扩展到五片串联电堆，在累积电荷、单片电压、冰量和电流密度约束下，依次研究恒流、线性升载和分段阶梯三种加载曲线。恒流通过单参数遍历选取，线性升载优化三个连续参数，阶梯加载进一步将切换次数纳入设计；三者均以五片单电池全部达到启动温度的时间为指标，并分析降低初温后的自启动可行性。
...
恒流加载是三种候选加载方式中最简单的一种：电流在短暂建立后保持不变，整个启动过程只由平台电流一个设计参数控制。因此直接遍历平台电流，比较每个取值的启动时间与约束状态，并基于选定曲线确定电堆的低温自启动能力。
...
\end{equation}
为避免低温初始阶段电流突变，采用 $2~\mathrm{s}$ 的短升流段，随后保持平台电流不变。固定 $j_0$ 与 $\tau_r$ 后，平台电流 $j_p$ 是唯一待选择参数。
目标是在电流、电量、电压和冰水含量约束下，最小化五片电池全部升至 $0~^{\circ}\mathrm{C}$ 的时间。

\subsubsection{求解方法}
\label{subsec:q2-cc-method}

在 $0.245$--$0.280~\mathrm{A\,cm^{-2}}$ 内选取等距主网格，并在约束边界附近加密，对每个平台电流直接运行同一五片电堆模型。\Cref{fig:q2-cc-trace}中间绘出成功启动点的平台电流与启动时间，两侧阴影概括同配置下已评价的低、高电流失败样本；仅中段横轴按连续电流刻度绘制。低电流样本升温缓慢、冰量累积，高电流样本较早出现较大压降；两侧的直接终止事件均为单片电压触及 $0.30~\mathrm{V}$。失败点不赋予启动时间，已评价区间之间尚有未测试的电流值，因此阴影不代表精确的可行域边界。

\begin{figure}[!t]
  \centering
  \includegraphics[width=14cm]{figures/q2_constant_trace_uniform.pdf}
  \caption{恒流加载的三类已评价结果（两端阴影为失败样本）}
  \label{fig:q2-cc-trace}
\end{figure}

\subsubsection{仿真设置}
\label{subsec:q2-cc-settings}

各候选曲线均采用初温 $-10~^{\circ}\mathrm{C}$、五片耦合模型和快速网格 $[2,3,4,4,2]$，主要设置见\Cref{tab:q2-cc-settings}。

\begin{table}[H]
  \centering
  \caption{恒流加载的主要仿真设置}
  \label{tab:q2-cc-settings}
  \begin{tabularx}{0.74\linewidth}{>{\raggedright\arraybackslash}X>{\centering\arraybackslash}X}
    \toprule
    设置项 & 取值\\
    \midrule
    初始温度/$^{\circ}\mathrm{C}$ & $-10$\\
    快速网格 & $[2,3,4,4,2]$\\
    结冰速率常数 $k_{\mathrm{freeze}}/\mathrm{s^{-1}}$ & $0.3$\\
    孔隙占用阈值 & $\varepsilon_l+\varepsilon_i<0.99$\\
    \bottomrule
  \end{tabularx}
\end{table}
...
遍历得到的最短可行启动时间为 $64.938125~\mathrm{s}$，对应平台电流 $j_p=0.267000~\mathrm{A\,cm^{-2}}$。其同网格复算结果见\Cref{tab:q2-cc-results}与\Cref{fig:q2-cc-comparison}。

\begin{table}[H]
  \centering
  \caption{选定恒流加载曲线的仿真结果}
  \label{tab:q2-cc-results}
  \begin{tabular}{lc}
    \toprule
    指标 & 选定曲线\\
    \midrule
    平台电流密度 $j_p/(\mathrm{A\,cm^{-2}})$ & 0.267000\\
    启动时间 $t_s/\mathrm{s}$ & 64.938125\\
    全程最低单片电压 $V_{\min}/\mathrm{V}$ & 0.323935\\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=14cm]{figures/q2_constant_comparison_uniform.pdf}
  \caption{选定恒流加载曲线的电流、温度、电压与最大冰体积分数}
  \label{fig:q2-cc-comparison}
\end{figure}

选定曲线的全程最低单片电压为 $0.323935~\mathrm{V}$，启动时累积电荷为 $17.0915~\mathrm{C\,cm^{-2}}$，均满足约束；下一网格点 $0.268~\mathrm{A\,cm^{-2}}$ 在启动前触及电压下限，表明进一步升高平台电流首先受单片电压约束限制。

\subsubsection{最低自启动初温}
\label{subsec:q2-cc-mintemp}

将遍历选定的 $j_p=0.267~\mathrm{A\,cm^{-2}}$ 曲线固定，仅改变电堆和环境初温进行二分试算。每次均采用同一快速网格与约束条件，不在不同初温下重新选取平台电流。

\begin{table}[H]
  \centering
  \caption{固定选定恒流曲线下的低温试算结果}
  ...
  \begin{tabularx}{0.94\linewidth}{>{\centering\arraybackslash}X>{\centering\arraybackslash}X>{\centering\arraybackslash}X>{\centering\arraybackslash}X>{\centering\arraybackslash}X>{\centering\arraybackslash}X}
    ...
    \midrule
    $-10$ & 成功 & 64.938125 & 0.323935 & 17.091480 & 0.366777\\
    $-11.125$ & 成功 & 73.776638 & 0.302882 & 19.451363 & 0.400421\\
    $-11.1625$ & 失败 & 64.342898 & 0.300000 & 16.932555 & 0.399718\\
    \bottomrule
  \end{tabularx}
\end{table}

在已测温度中，$-11.125~^{\circ}\mathrm{C}$ 是最冷的成功点，$-11.1625~^{\circ}\mathrm{C}$ 为相邻失败点。若临界点附近的成功性随初温单调变化，则这条固定曲线的临界初温位于二者之间。失败时第 1 片首先触及 $0.30~\mathrm{V}$ 电压下限，累计电荷和冰量均未达到上限，表明端部单电池的电压约束决定该曲线的低温边界。以上仅为快速网格数值试算结果，不代表允许重新设计加载曲线时的全局最低初温。
...
搜索范围取
\begin{equation}
  0\le j_0\le0.20,\qquad
  0\le r\le0.020,\qquad
  0.30\le j_p\le0.50,
  \label{eq:q2-search-domain}
\end{equation}
为提高代理模型的数值稳定性，将三个变量归一化到 $[0,1]^3$：
\begin{equation}
  x_1=\frac{j_0}{0.20},\qquad
  x_2=\frac{r}{0.020},\qquad
  x_3=\frac{j_p-0.30}{0.20}.
\end{equation}
...
对已评价样本分别建立两个高斯过程代理模型：可行性模型以启动成功或失败的二元标签为训练目标，时间模型只拟合成功样本的 $t_s$。找到可行点后，每轮以当前最佳真实可行点为中心，在半径为 $R$ 的球内生成检查点；根据预测可行概率与预测启动时间选出较优检查点，并用其包络得到本轮超矩形信赖域。在该区域内先用拉丁超立方生成候选，再最大化
\begin{equation}
  \log a(\bm{x})=
...
在式\eqref{eq:q2-search-domain}给定的参数范围内，先生成 $32$ 个空间填充样本，以获得可行性和启动时间的初始观测；随后由代理模型依次选择 $32$ 个评价点。搜索不依赖预设的参考加载曲线。
...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}[1]
    ...
    \STATE 在式\eqref{eq:q2-search-domain}的范围内生成空间填充初始样本，并逐一真实仿真
    \WHILE{真实模型评价次数未达到上限}
...
\Cref{fig:q2-linear-trace}展示了从零开始的 $64$ 次模型评价：前 $32$ 次为空间填充采样，后 $32$ 次根据代理模型选点。可行方案逐渐集中于启动较快的参数区域，最短启动时间出现在第 $61$ 次评价。图中的上下虚线表示当轮搜索范围。

\begin{figure}[!t]
  \centering
  \includegraphics[width=14cm]{figures/q2_linear_trace_uniform.pdf}
  \caption{线性升载参数的搜索过程}
  \label{fig:q2-linear-trace}
\end{figure}

\Cref{fig:q2-linear-parameter-cloud}只展示后 $32$ 次代理模型选点在 $(j_0,r,j_p)$ 空间中的搜索路径。连线按评价顺序连接候选点，成功点按启动时间 $t_s$ 着色，未成功点为灰色；方形标记为第 $33$ 次评价，星形标记为最优的第 $61$ 次评价。半透明球标示末期样本集中的搜索邻域，并非球内所有策略都已验证可行。

\begin{figure}[H]
  \centering
  \includegraphics[width=14cm]{figures/q2_linear_parameter_cloud_uniform.pdf}
  \caption{线性升载代理模型选点的空间轨迹与末期搜索邻域（灰色点为未成功评价）}
  \label{fig:q2-linear-parameter-cloud}
\end{figure}

\subsubsection{仿真设置}

各候选方案在相同模型和快速网格上计算，设置见\Cref{tab:q2-settings}。

\begin{table}[H]
  \centering
  \caption{问题二线性加载的主要仿真设置}
  \label{tab:q2-settings}
  \begin{tabularx}{0.78\linewidth}{>{\raggedright\arraybackslash}X>{\centering\arraybackslash}p{0.47\linewidth}}
    \toprule
    设置项 & 取值\\
    \midrule
    初始温度 & $-10~^{\circ}\mathrm{C}$\\
    单片快速网格 & $[2,3,4,4,2]$\\
    相变与冰覆盖参数 & $k_{\mathrm{freeze}}=0.4~\mathrm{s^{-1}}$，$\gamma_{\mathrm{ice}}=3.5$\\
    端板与双极板处理 & 独立端板，共享双极板计数\\
    约束阈值 & $j\le0.50$，$V_k\ge0.30$，$q_{\mathrm{use}}\le20$\\
    冰与孔隙阈值 & $\varepsilon_{\mathrm{ice}}<0.99$，$\varepsilon_l+\varepsilon_i<0.98$\\
    \bottomrule
  \end{tabularx}
\end{table}
...
真实模型搜索得到的最短可行曲线为
\begin{equation}
  j_0=0.141964965~\mathrm{A\,cm^{-2}},\qquad
  r=0.007203574~\mathrm{A\,(cm^2\,s)^{-1}},\qquad
  j_p=0.476005171~\mathrm{A\,cm^{-2}}.
  \label{eq:q2-best-strategy}
\end{equation}
其复算结果见\Cref{tab:q2-results}与\Cref{fig:q2-linear-comparison}。

\begin{table}[H]
  \centering
  \caption{选定线性升载曲线的仿真结果}
  \label{tab:q2-results}
  \begin{tabular}{lc}
    \toprule
    指标 & 选定曲线\\
    \midrule
    $j_0/(\mathrm{A\,cm^{-2}})$ & 0.141965\\
    $r/(\mathrm{A\,(cm^2\,s)^{-1}})$ & 0.007204\\
    $j_p/(\mathrm{A\,cm^{-2}})$ & 0.476005\\
    启动时间 $t_s/\mathrm{s}$ & 40.558338\\
    启动时累积电荷 $q_{\mathrm{use}}/(\mathrm{C\,cm^{-2}})$ & 11.682746\\
    全程最低单片电压 $V_{\min}/\mathrm{V}$ & 0.308672\\
    全程最大局部冰体积分数 & 0.331532\\
    启动时最大局部冰体积分数 & 0.178274\\
    启动时电流密度 $j(t_s)/(\mathrm{A\,cm^{-2}})$ & 0.434130\\
    \bottomrule
  \end{tabular}
\end{table}

选定曲线从 $0.1420~\mathrm{A\,cm^{-2}}$ 起步，以 $0.007204~\mathrm{A\,(cm^2\,s)^{-1}}$ 持续升载，使电流增长与膜水合、温升和融冰过程相匹配，同时保持单片电压不低于下限。

\Cref{fig:q2-linear-comparison}左上图显示，选定曲线的理论平台到达时间为
\begin{equation}
  t_p=\frac{j_p-j_0}{r}=46.3715~\mathrm{s},
\end{equation}
模型在 $40.5583~\mathrm{s}$ 已启动，此时 $j=0.434130~\mathrm{A\,cm^{-2}}$。因此，曲线在启动前尚未进入平台阶段，主要通过持续升载加热。

温度结果表明，最冷单片在 $40.5583~\mathrm{s}$ 达到 $0~^{\circ}\mathrm{C}$。启动时五片平均温度依次约为 $0$、$4.834$、$6.358$、$4.997$ 和 $0.347~^{\circ}\mathrm{C}$，中部单片温升更快，第一片仍是决定启动时刻的热约束单片。此时左右端板温度约为 $-6.17$ 和 $-6.13~^{\circ}\mathrm{C}$，说明将端板设为独立热节点是必要的；若强制端板与端部单片同温，将明显改变端部热惯性与启动判定。

电压约束是继续缩短启动时间的主要瓶颈。选定曲线的全程最低单片电压为 $0.308672$ V，出现在约 $31.0$ s 的第3片，距离 $0.30$ V 下限仅 $0.008672$ V。此后尽管电流仍在上升，温升、膜水合改善及局部融冰使电压重新回升。启动时电流距离上限尚有约 $0.0659$ A/cm$^2$ 的余量，累积电荷只使用上限的 $58.41\%$，冰体积分数和孔隙占用也远未达到失败阈值。因此，在当前模型参数下，简单提高初始电流或升载速率首先会触及单片低电压边界，而不是电荷或冰量上限。

冰量曲线显示，选定曲线的最大局部冰体积分数在约 $38.5~\mathrm{s}$ 达到 $0.3315$，随后随局部温度接近冰点而快速下降，启动时降至 $0.1783$。全程最大冰量出现在第1片，与其端部散热较强、温升最慢相一致；最低电压却出现在第3片，说明热约束、冰约束和电压约束不一定由同一片电池控制。加载策略必须对五片电池的完整历史同时检查，不能只依据平均堆温或总电压判断启动是否安全。

\begin{figure}[H]
  \centering
  \includegraphics[width=14cm]{figures/q2_linear_comparison_uniform.pdf}
  \caption{选定线性升载曲线的电流、温度、电压与冰量}
  \label{fig:q2-linear-comparison}
\end{figure}

\subsubsection{兼顾启动速度的低温可行性检验}
\label{subsec:q2-linear-mintemp}

上述 $40.5583~\mathrm{s}$ 曲线针对 $-10~^{\circ}\mathrm{C}$ 下的启动时间最小化。若进一步考察更低初温，可允许该工况的启动时间至多增加 $5\%$，即将时间上限设为 $1.05\times40.558338=42.586255~\mathrm{s}$。另行考察的低温检验曲线在 $-10~^{\circ}\mathrm{C}$ 下满足此限，其固定形式为
\begin{equation}
  j(t)=\min\{0.144980840+0.006798555t,\;0.478303370\}
  \quad(\mathrm{A\,cm^{-2}}).
  \label{eq:q2-linear-lowtemp-strategy}
\end{equation}
它不是前述 $-10~^{\circ}\mathrm{C}$ 最快曲线：同口径下的启动时间约为 $41.992~\mathrm{s}$，比最快曲线增加约 $3.53\%$，仍在 $5\%$ 容差内。检验时固定式\eqref{eq:q2-linear-lowtemp-strategy}的全部加载参数，只将五片电池初温与环境温度同步改为待测温度。用更新后的独立代码复算结果见\Cref{tab:q2-linear-lowtemp}。

\begin{table}[H]
  \centering
  \caption{固定线性加载曲线在不同初温下的快速网格复算}
  \label{tab:q2-linear-lowtemp}
  \begin{tabular}{ccccc}
    \toprule
    \shortstack{初始及环境温度\\$T_0/^{\circ}\mathrm{C}$} & 结果 & \shortstack{启动或终止时间\\$t/\mathrm{s}$} & \shortstack{最低单片电压\\$V_{\min}/\mathrm{V}$} & \shortstack{终止时累计电荷\\$q_{\mathrm{use}}/(\mathrm{C\,cm^{-2}})$}\\
    \midrule
    $-10$ & 成功 & 41.9918 & 0.39734 & 12.0820\\
    $-11$ & 成功 & 44.6723 & 0.35764 & 13.2603\\
    $-12$ & 第4片失压 & 39.3849 & 0.30000 & 10.9829\\
    \bottomrule
  \end{tabular}
\end{table}

$-12~^{\circ}\mathrm{C}$ 时，第4片单电池先触及 $0.30~\mathrm{V}$ 下限；此时累计电荷仍低于 $20~\mathrm{C\,cm^{-2}}$，最大局部冰体积分数约为 $0.3400$，也未达到冰量上限。因此，这条固定曲线在该已测温度下的直接失败原因是单片失压，而不是电荷耗尽或冰体积分数越限。$-11~^{\circ}\mathrm{C}$ 是目前已复算的最冷成功点，$-12~^{\circ}\mathrm{C}$ 是已测失败点；两点之间尚未二分，也未在每个初温重新优化曲线，不能把 $-11~^{\circ}\mathrm{C}$ 称为精确或全局最低自启动初温。以上差异仅为当前快速网格的模型仿真结果。
...
分段阶梯加载通过改变各时段电流，协调产热与产水。与仅搜索一个平台电流的恒流策略和搜索三个连续参数的线性升载策略不同，此处还需确定电流切换次数，即曲线结构本身也是设计变量。

\subsubsection{优化模型}
\label{subsec:q2-step-model}

令 $K$ 为电流切换次数，则曲线有 $K+1$ 个电流平台，其表达式为
\begin{equation}
  j(t;K,\bm{\theta}_K)=j_i,\quad t_{i-1}\le t<t_i,
  \quad i=1,\ldots,K+1,
  \qquad
  \bm{\theta}_K=(j_1,\ldots,j_{K+1},t_1,\ldots,t_K),
  \label{eq:q2-step-current}
\end{equation}
其中 $t_0=0$，$t_{K+1}=\infty$，$0\le j_i\le0.5~\mathrm{A\,cm^{-2}}$，且 $t_1<\cdots<t_K$。因此，对给定 $K$ 有 $2K+1$ 个连续参数；电流平台不限定为单调递增。优化问题为
\begin{equation}
  \min_{K,\bm{\theta}_K}\ t_s(K,\bm{\theta}_K;-10~^{\circ}\mathrm{C}),
  \qquad
  \text{s.t. 电流、电量、电压及冰量约束},
  \label{eq:q2-step-decode}
\end{equation}
其中运行约束与式\eqref{eq:q2-objective}一致。计算时将 $K$ 限于 $1$ 至 $7$，作为有限预算下的搜索范围，而非预先指定阶梯数。

\subsubsection{求解方法}
\label{subsec:q2-step-method}

采用变结构代理辅助搜索：先评价初始候选曲线，再对相邻平台合并、现有平台拆分，并微调电流和切换时刻，形成不同 $K$ 的候选集。以高斯过程估计候选的启动时间，优先计算预测启动较快且与已有样本有差异的曲线；每个入选候选仍由五片模型真实仿真并检查约束，再更新代理模型和当前最优解。合并和拆分使搜索能够改变参数维数，而不必先固定阶梯数。该有限预算搜索给出可行方案，不声称已找到全局最优阶梯数。

\subsubsection{仿真设置}
\label{subsec:q2-step-settings}

优化工况的五片电池和环境初温均为 $-10~^{\circ}\mathrm{C}$，采用 $k_{\mathrm{freeze}}=0.4~\mathrm{s^{-1}}$、$\gamma_{\mathrm{ice}}=3.5$ 和快速网格 $[2,3,4,4,2]$；电流、电量及单片最低电压约束分别为 $0.5~\mathrm{A\,cm^{-2}}$、$20~\mathrm{C\,cm^{-2}}$ 和 $0.30~\mathrm{V}$。

\subsubsection{优化结果与约束分析}
\label{subsec:q2-step-results}

搜索记录包含两条初始候选曲线和 $16$ 次新增模型评价；实际评价覆盖 $K=4,5,6$，各结构中最快的可行启动时间分别为 $36.6056$、$34.9724$ 和 $36.5873~\mathrm{s}$。有限预算内最短的可行启动时间出现在 $K=5$，即六个电流平台和五个切换时刻，共 $11$ 个连续参数。\Cref{tab:q2-step-results}给出该曲线的模型计算结果，\Cref{fig:q2-step-comparison}展示相应动态过程。

\begin{table}[H]
  \centering
  \caption{选定阶梯加载曲线的仿真结果}
  \label{tab:q2-step-results}
  \small
  \begin{tabularx}{0.85\linewidth}{>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}p{0.48\linewidth}}
    \toprule
    指标 & 选定曲线\\
    \midrule
    切换次数 $K$ & 5\\
    档位电流/$(\mathrm{A\,cm^{-2}})$ & 0.1752, 0.2914, 0.2427, 0.2618, 0.5000, 0.4420\\
    切换时刻/s & 2.5713, 12.1507, 20.5979, 27.0905, 31.0000\\
    $-10~^{\circ}\mathrm{C}$ 启动时间/s & 34.972396\\
    $-10~^{\circ}\mathrm{C}$ 最低单片电压/V & 0.314724\\
    $-10~^{\circ}\mathrm{C}$ 累积电荷/$(\mathrm{C\,cm^{-2}})$ & 10.702447\\
    $-10~^{\circ}\mathrm{C}$ 最大局部冰体积分数 & 0.296155\\
    \bottomrule
  \end{tabularx}
\end{table}

搜索允许阶梯数变化，本次最短可行方案对应 $K=5$。这只是有限评价预算下的结果，不表示预先把阶梯数固定为五次切换。

\begin{figure}[H]
  \centering
  \includegraphics[width=14cm]{figures/q2_step_comparison_uniform.pdf}
  \caption{选定阶梯加载曲线在 $-10~^{\circ}\mathrm{C}$ 下的电流、温度、电压与冰量}
  \label{fig:q2-step-comparison}
\end{figure}

如\Cref{fig:q2-step-comparison}所示，选定曲线依次调整电流平台，最冷单片在 $34.972396~\mathrm{s}$ 到达 $0~^{\circ}\mathrm{C}$。其全程最低单片电压为 $0.314724~\mathrm{V}$，最大局部冰体积分数为 $0.296155$，均满足所设约束。

\subsubsection{低温可行性验证}
\label{subsec:q2-step-mintemp}

将\Cref{tab:q2-step-results}中的选定曲线固定、仅降低电堆和环境初温进行复算。该曲线在 $-10~^{\circ}\mathrm{C}$ 可启动，而在 $-10.0375~^{\circ}\mathrm{C}$ 已先触及 $0.30~\mathrm{V}$ 单片电压下限；$-10.075$、$-10.15$、$-10.3$、$-10.6$ 和 $-11.2~^{\circ}\mathrm{C}$ 的试算同样未能启动。若临界点附近的成功性随初温单调变化，其临界初温位于 $-10.0375$ 与 $-10~^{\circ}\mathrm{C}$ 之间。这表明以 $-10~^{\circ}\mathrm{C}$ 启动时间最短为目标所选的阶梯曲线，虽启动迅速，但对初温下降较敏感；最低初温的结论仅适用于这条固定曲线。

\subsection{三种加载策略的结果汇总}

\Cref{tab:q2-strategy-summary}汇总 $-10~^{\circ}\mathrm{C}$ 下三条选定曲线的五片电堆仿真结果。恒流曲线为前 $2~\mathrm{s}$ 升流后恒流；线性曲线采用式\eqref{eq:q2-linear-lowtemp-strategy}的低温检验方案，表中电流为启动前峰值；阶梯曲线为\Cref{tab:q2-step-results}中的变结构搜索选定方案。恒流采用 $k_{\mathrm{freeze}}=0.3~\mathrm{s^{-1}}$，其余两种采用 $0.4~\mathrm{s^{-1}}$，低温约束也不同，故仅汇总设计结果，不作严格优劣排序。

\begin{table}[H]
  \centering
  \caption{三种选定加载曲线在 $-10~^{\circ}\mathrm{C}$ 下的仿真结果}
  \label{tab:q2-strategy-summary}
  \footnotesize
  \setlength{\tabcolsep}{2.5pt}
  \begin{tabular}{lcccccc}
    \toprule
    加载策略 & 启动时间/s & 累积电荷/$(\mathrm{C\,cm^{-2}})$ & 最大电流/$(\mathrm{A\,cm^{-2}})$ & 最低电压/V & 最大冰体积分数 & 结果\\
    \midrule
    短时升流后恒流 & 64.938125 & 17.091480 & 0.267000 & 0.323935 & 0.366777 & 成功\\
    线性升载 & 41.991669 & 12.081952 & 0.430464 & 0.397340 & 0.339889 & 成功\\
    可变结构阶梯加载 & 34.972396 & 10.702447 & 0.500000 & 0.314724 & 0.296155 & 成功\\
    \bottomrule
  \end{tabular}
\end{table}
...
\subsection{辅助加热模型与优化目标}

在五片单电池水--热--电耦合模型中，为每片双极板设置独立电热丝。设第 $k$ 片的恒定加热功率密度为 $q_k$，五片共用计划停热时刻 $t_h$，决策向量为 $\bm x=(q_1,\ldots,q_5,t_h)$，其中 $0\le q_k\le1~\mathrm{W\,cm^{-2}}$。单片面积 $A=25~\mathrm{cm^2}$，故实际输入功率为
\begin{equation}
  P_{h,k}(t)=Aq_k\,\bm 1_{[0,t_h)}(t).
  \label{eq:q3-heater}
\end{equation}
原模型未单设电热丝和双极板的空间温度节点；计算时将该功率守恒地折算为单片一维温度场中的均匀热源，即 $S_{h,k}=10^4q_k/L_{\mathrm{1D}}$（加热开启时），其中 $L_{\mathrm{1D}}$ 为一维计算域厚度。热传导、相变及电化学产热仍按前文耦合方程求解，端板温度作为独立状态计算。这个折算保留每片输入的总热量，但不解析电热丝附近的局部温差。电堆的热耦合结构见\Cref{fig:q3-thermal-schematic}。

\begin{figure}[H]
  \centering
  \includegraphics[width=14cm]{figures/q3_thermal_network.png}
  \caption{五片电堆辅助加热与热耦合示意（协同加热约 $30~\mathrm{s}$，双极板色彩按相邻求解节点插值）}
  \label{fig:q3-thermal-schematic}
\end{figure}

纯预加热阶段取 $j(t)=0$，五片平均温度均越过 $0~^{\circ}\mathrm{C}$ 即记为本问的预热启动完成，表中时间不再额外叠加后续带载时间。恒定功率协同启动从 $t=0$ 同时通电和加热，按照题定曲线
\begin{equation}
  j(t)=\min\{0.005t,\,0.3\}\quad(\mathrm{A\,cm^{-2}})
  \label{eq:q3-current}
\end{equation}
升载；恒定的是电热丝功率，而非电堆输出功率。两种工况的电堆各部件及环境初温均为 $-30~^{\circ}\mathrm{C}$。

若 $t_s$ 为首次满足五片同时启动条件的时刻，实际计入能耗的加热时间为 $t_{h,\mathrm{used}}=\min(t_h,t_s)$。由此，单片及电堆辅助电能分别为
\begin{equation}
  E_k=Aq_k t_{h,\mathrm{used}},\qquad
  \min_{\bm x}\ E_{\mathrm{aux}}(\bm x)
  =\min_{\bm x}\sum_{k=1}^{5}E_k.
  \label{eq:q3-objective}
\end{equation}
最小化目标仅为电热丝消耗的电能，不含氢耗及其他辅机能耗。可行性沿用问题二的五片平均温度过零、单片电压不低于 $0.30~\mathrm{V}$、最大局部冰体积分数低于 $0.99$、单位面积累计电荷不超过 $20~\mathrm{C\,cm^{-2}}$ 等条件。协同策略在计划停热后仍继续求解，直至启动成功或触发约束；因此一般不能将 $t_h$ 直接当作 $t_s$。

\subsection{参数搜索与数值设置}

针对纯预热，先比较保存的功率分配候选，再在可行边界附近缩小功率与过零时刻步长，去除启动成功后的冗余加热时间。针对协同启动，在五片功率与共同停热时刻组成的六维空间内进行差分进化搜索，并补充端片、中间片减功率及提前停热等定向检验。每个候选均调用五片瞬态模型，以真实启动判定和辅助能耗更新可行方案记录；有限次数搜索并不构成全局最优性证明。

两类加热方案的候选生成方式不同，但共用瞬态仿真、约束判定与能耗评价步骤，整体求解流程见\Cref{fig:q3-algorithm-flow}。

\begin{figure}[H]
  \centering
  \includegraphics[width=14cm]{figures/q3_algorithm_flow.pdf}
  \caption{问题三两种辅助加热策略的参数搜索与仿真评价流程}
  \label{fig:q3-algorithm-flow}
\end{figure}

仿真沿用 $[2,3,4,4,2]$ 的五层控制体网格。对选定方案采用更严格的时间积分设置（相对容差 $10^{-6}$、绝对容差 $10^{-10}$、最大时间步 $0.0125~\mathrm{s}$）复核。这一复核改进时间积分精度，但不等同于空间网格收敛检验。

\subsection{优化结果与机理分析}

\Cref{tab:q3-results}给出两种策略在当前模型及已执行搜索中的最低辅助能耗可行记录。两种记录都在达到启动条件前保持加热，因此实际加热时间与总启动时间相同。协同分配的第五片功率密度约为 $0.9956~\mathrm{W\,cm^{-2}}$；表中舍入仅供展示，复算采用保存的完整数值。

\begin{table}[H]
  \centering
  \caption{两种辅助冷启动策略的模型计算结果}
  \label{tab:q3-results}
  \begin{tabular}{lcc}
    \toprule
    指标 & 纯预加热 & 恒定功率协同\\
    \midrule
    功率密度分配 $\bm q/(\mathrm{W\,cm^{-2}})$ & $(1,1,1,1,1)$ & $(1,1,1,1,0.9956)$\\
    加热持续时间 $t_{h,\mathrm{used}}/\mathrm{s}$ & 33.5163 & 30.2539\\
    第1片辅助电能 $E_1/\mathrm{J}$ & 837.907 & 756.348\\
    第2片辅助电能 $E_2/\mathrm{J}$ & 837.907 & 756.348\\
    第3片辅助电能 $E_3/\mathrm{J}$ & 837.907 & 756.348\\
    第4片辅助电能 $E_4/\mathrm{J}$ & 837.907 & 756.348\\
    第5片辅助电能 $E_5/\mathrm{J}$ & 837.907 & 753.011\\
    总辅助电能 $E_{\mathrm{aux}}/\mathrm{J}$ & 4189.537 & 3778.402\\
    总启动时间 $t_s/\mathrm{s}$ & 33.5163 & 30.2539\\
    全程输出最大局部冰体积分数 & 0.011603 & 0.119673\\
    启动结果 & 成功 & 成功\\
    \bottomrule
  \end{tabular}
\end{table}

协同方案相对纯预热少用辅助电能 $411.135~\mathrm{J}$，降幅 $9.81\%$；启动提前 $3.2624~\mathrm{s}$，缩短 $9.73\%$。\Cref{fig:q3-timecourse}中的最冷单片温度曲线表明两端片决定最终过零时刻。协同加热在获得额外反应热的同时产生水，故端片最大局部冰体积分数达到 $0.119673$，高于纯预热的 $0.011603$。因此本组计算并非协同策略在能耗、速度和冰堵抑制三方面均占优。纯预热虽无反应电流，模型中的初始水合与水迁移仍可带来少量结冰；协同方案的全程最低单片电压为 $0.6308~\mathrm{V}$，未触及 $0.30~\mathrm{V}$ 下限。

\begin{figure}[H]
  \centering
  \includegraphics[width=14cm]{figures/q3_strategy_timecourse.pdf}
  \caption{辅助冷启动过程的温度、能耗、端片冰量及电压}
  \label{fig:q3-timecourse}
\end{figure}

\Cref{fig:q3-temperature}进一步显示，启动时中间片明显暖于端片，而两侧端板仍低于冰点。以协同方案为例，第3片平均温度约 $19.24~^{\circ}\mathrm{C}$，第1、5片仅约 $0.01~^{\circ}\mathrm{C}$，两侧端板约为 $-18.86~^{\circ}\mathrm{C}$。端板吸热及片间导热使两端片成为加热瓶颈，也解释了已发现的低能耗解仍接近五片满功率。图中横轴为端板和单片节点顺序，并非连续空间距离。

\begin{figure}[H]
  \centering
  \includegraphics[width=14cm]{figures/q3_terminal_temperature.pdf}
  \caption{两种策略各自启动完成时的端板与五片单电池平均温度}
  \label{fig:q3-temperature}
\end{figure}

\subsection{结果适用范围}

以上数值属于资料包原版模型的有限搜索结果，而非实验实测或已证明的全局最优。当前协同方案相对同精度五片全满功率协同基准仅节约约 $2.059~\mathrm{J}$，这一微小差别尚未接受空间网格误差检验，故不将第五片 $0.9956$ 的细小偏差解释为确定的物理最优分配。还须注意，本节保存代码采用 $E_a=8000~\mathrm{J\,mol^{-1}}$，与问题一正文的 $67000~\mathrm{J\,mol^{-1}}$ 参数口径不一致，并在可行性判定中加入了题目冰量阈值以外的孔隙占用限制 $0.98$。因此表中数值应理解为该原版实现下的条件性结果；若与前文统一参数及约束，需重新进行优化和复算，不能直接沿用此表。
